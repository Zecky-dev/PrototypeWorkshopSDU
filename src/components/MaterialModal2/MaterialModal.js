...
const MaterialModal = ({isVisible,setModalVisible,type,data}) => {
    ...
    const editMaterial = async (roomID,materialID,materialName,materialUnit,materialDescription) => {
      try {
        ...
        <View style={styles.container}>
          <Formik
            initialValues={{
              ...
              materialDescription: data["0"]? data["0"].materialDescription : "",
            }}
            validationSchema={materialValidationSchema}
            onSubmit={
              type === "add" ? (values) => addMaterial(data.id,data.title,values.materialName,values.materialUnit,values.materialDescription) :
              type === "edit" ? (values) => editMaterial(data.id,data["0"].materialID,values.materialName,values.materialUnit,values.materialDescription) : null
            }>
            ...
            <ScrollView style={styles.innerContainer}>
              ...
              <View style={styles['input'].container}>
                ...
                <CustomTextInput value={values.materialName} onChangeText={handleChange('materialName')}
                />
              </View>
              ...
              <View style={styles['input'].container}>
                ...
                <CustomTextInput value={values.materialUnit} onChangeText={handleChange('materialUnit')} type={'numeric'}/>
              </View>

              {/* material unit*/}
              <View style={styles['input'].container}>
                <Text style={styles['input'].label}>Materyal Açıklaması</Text>
                <CustomTextInput value={values.materialDescription} multiline={true} onChangeText={handleChange('materialDescription')}/>
              </View>
